...
import (
	...
	"github.com/dmitry-drozdov/gqlgen/plugin/modelgen"
)
...
In `resolver.go`, add `//go:generate go run generate.go` (or replace `//go:generate go run github.com/dmitry-drozdov/gqlgen generate` if you have it there). 
